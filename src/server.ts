...
const app = buildApp({ service: built.service, stripe: built.stripe, apiKey: built.apiKey, authSecret: built.authSecret, stripeLive: !built.sandbox.stripe, mailer: built.mailer, smsAuthToken: built.smsAuthToken });
...
serve({ fetch: app.fetch, port }, (info) => {
  ...
  console.log(
    `    store: ${built.store} · stripe: ${mode(built.sandbox.stripe)} · negotiator: ${mode(built.sandbox.negotiator)} · sms: ${mode(built.sandbox.sms)} · auth: ${
