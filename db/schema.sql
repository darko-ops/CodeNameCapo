...
create table if not exists bouncr.plans (
  ...
  discovery_jsonb jsonb  -- Vini/discovery config, renderer-only (null => off)
);
-- Idempotent adds for existing deployments (the columns are new).
alter table bouncr.plans add column if not exists application_fee_percent numeric;
alter table bouncr.plans add column if not exists discovery_jsonb jsonb;
